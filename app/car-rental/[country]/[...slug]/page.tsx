...
export async function generateStaticParams() {
  const [states, cities] = await Promise.all([getStatesIndex(), getCitiesIndex()]);
  ...
  const stateCityParams = cities
    .filter(
      (c) => c.has_state_layer && c.state_slug && c.publication_state === "indexed"
    )
    .map((c) => ({
      country: c.country_slug,
      slug: [c.state_slug as string, c.city_slug],
    }));

  return [...stateParams, ...stateCityParams];
}
...
export async function generateMetadata({
  ...
  if (!city || city.publication_state !== "indexed") {
    return { title: "Not Found" };
  }
...
export default async function StateOrStateCityPage({
  params,
}: {
  params: Promise<{ country: string; slug: string[] }>;
}) {
  ...
  if (slug.length === 1) {
    ...
    const [state, cities, airports, guides] = await Promise.all([
  ...
  const [city, statesIndex, guides] = await Promise.all([
    getCityBySlug(citySlug, countrySlug, stateSlug),
    getStatesIndex(),
    getGuidesByCountry(countrySlug),
  ]);
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {nearbyCities.slice(0, 5).map((nearby) => (
                      <li
                        key={`${nearby.country_slug}-${nearby.state_slug ?? "nostate"}-${nearby.city_slug}`}
                      >
                        <Link
