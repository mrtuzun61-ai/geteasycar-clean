...
function getCityHeroImage(citySlug: string, cityName: string, countryName: string): string {
  return (
...
function stateAirports(
  airports: AirportIndexEntry[],
  citySlug: string
): AirportIndexEntry[] {
  return airports.filter((a) => a.city_slug === citySlug);
}
...
export async function generateStaticParams() {
  ...
  return cities
    .filter((c) => c.has_state_layer && c.state_slug && c.publication_state === "indexed")
    .map((c) => ({
...
export default async function CityStatePage({
  ...
  const [city, country, statesIndex, allCountryAirports, guides] = await Promise.all([
    getCityBySlug(citySlug, countrySlug, stateSlug),
    getCountryBySlug(countrySlug),
    getStatesIndex(),
  ...
  stateAirports(
    allCountryAirports.filter((a) => a.publication_state === "indexed"),
    citySlug
  );
        ...
        <section className="border-b border-slate-100 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-10 text-sm font-medium text-slate-600">
              {[
                { icon: "✓", label: "Free Cancellation Available" },
                { icon: "✓", label: "No Hidden Platform Fees" },
                { icon: "✓", label: "Trusted Rental Partners" },
              ].map(({ icon, label }) => (
        ...
        <section className="py-12 sm:py-14 bg-blue-50 border-y border-blue-100">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p className="text-slate-600 text-base mb-7 leading-relaxed">
              Find and compare rental vehicles across {city.city_name} and surrounding
              areas. Connect directly with trusted suppliers — no booking fees.
            </p>
            <a
              ...
              <div className="lg:col-span-1">
                ...
                <div className="flex flex-col gap-3 mt-6">
                  ...
                  <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-400 font-medium">Min. Driver Age</p>
                      <p className="text-sm font-semibold text-slate-800">
                  ...
                  <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-800">{country.currency_code}</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  !country.parking_notes && (
                    <div>
                      ...
                      <p className="text-slate-600 text-sm leading-relaxed">
                        ...
                        recommended for visitors whose license is not in the
                        local language.
                      </p>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {nearbyCities.slice(0, 5).map((nearby) => {
                      const nearbyUrl =
                        nearby.has_state_layer && nearby.state_slug
                          ? `/car-rental/${nearby.country_slug}/${nearby.state_slug}/${nearby.city_slug}/`
                          : `/car-rental/${nearby.country_slug}/${nearby.city_slug}/`;
                      return (
